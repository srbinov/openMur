...
2. Ensure openMur is listed and enabled
...
3. Ensure openMur is listed and enabled
...
4. Clear model cache: `rm -rf ~/.cache/openmur/whisper-models`
...
3. Restart openMur after installing

openMur tries clipboard methods in order: `wl-copy` (most reliable) → renderer `navigator.clipboard` → X11 fallback.
...
4. Expect the chooser to appear on Linux while openMur is using the standard browser portal path; some desktops may support more persistent portal behavior later, but fallback capture should still work
...
**macOS:**
1. Grant Screen Recording permission: System Settings → Privacy & Security → Screen Recording → enable openMur
...
1. Check the system tray (click the `^` caret) for the openMur icon
2. Run with debug logging: `openMur.exe --log-level=debug`
3. Try disabling GPU acceleration: `openMur.exe --disable-gpu`

**Antivirus / Windows Defender blocking binaries:**

whisper.cpp and FFmpeg may be quarantined silently. Add openMur to exclusions: Settings → Virus & threat protection → Exclusions.

**Permission errors:**

Right-click openMur → Run as administrator (or set permanently in Properties → Compatibility).

**Firewall blocking cloud mode:**

Allow openMur through Windows Firewall when using cloud transcription providers.

**Complete reset (after uninstalling):**

```batch
rd /s /q "%APPDATA%\openMur"
rd /s /q "%LOCALAPPDATA%\openMur"
```

Then reinstall.

**Logs location:** `%APPDATA%\openMur\logs\`
...
3. Open an issue at https://github.com/openMur/openmur/issues with:
   - OS version
   - openMur version
